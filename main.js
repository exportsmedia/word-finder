document.addEventListener('alpine:init', () => {
    Alpine.data('initData', () => ({
        ...
        timer: '',
        showHelp: false,
        toastText: 'Not the correct answer, try again!',
        date: new Date().toLocaleString("en-US", { timeZone: "America/Los_Angeles" }),
        checkGame(action, e) {
            e.preventDefault();
            this.stats.lastPlayed = new Date().toISOString().split('T')[0];

            if(action == 'guess' && (this.selectedkey == null || this.selectedkey.length == 0 || this.selectedlabel == null || this.selectedlabel.length == 0)) {
                this.updateToast("Please enter a guess!");
                return;
            }

            // check if word is valid
            // update word and percentage found if yes
            // show error if no
            // if found all words hide input show stat modal

            if(action == 'guess' && this.selectedlabel.toLowerCase() == this.answer.toLowerCase()) {
                localStorage.setItem('gameOver', true)
                this.gameOver = true;
                this.showStats = true;
                this.solvedOn = this.guessNumber + 1;
        ...
        share() {
            ...
            //let clues = Number(this.guessNumber) + 1;
            let guesses = [];

            if(this.solvedGame == true) {
                share += 'Guessed in ' + this.guessNumber + "!\n\n";
            } else {
                share += "Did not solve\n\n";
            }

            // for (let step = 0; step <= this.cluesLength; step++) {
            //     guesses[step] = '⬜';
            // }

            for (let step = 0; step < (clues - 1); step++) {
                guesses[step] = '⬛';
            }

            guesses[(this.guessNumber - 1)] = '🟩';

            for (let step = 0; step < guesses.length; step++) {
                share += guesses[step];
            }

            // let solvedOn = this.solvedOn;
            // if(this.solvedOn == this.clues.length) {
            //     solvedOn = 'x';
            // }

            // if(this.solvedGame == true) {
            //     share += ' ' + this.guessNumber + '/' + (this.clues.length + 1) + "\n\n" + document.URL;
            // } else {
            //     share += ' x/' + (this.clues.length + 1) + "\n\n" + document.URL;
            // }
